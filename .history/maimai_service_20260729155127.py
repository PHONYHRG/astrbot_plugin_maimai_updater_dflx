...
ACCEPTED_OFFICIAL_LOGIN_RETURN_CODES = {1, 100, 104}
...
class MaimaiService:
    ...
    # ====== 新增落雪方法（使用个人 API）======
    async def _upload_to_luoxue(self, scores: list[dict], api_key: str) -> dict:
        """
        使用落雪个人 API 上传成绩（操作的是 API 密钥所属用户自己的数据）
        """
    ...
    async def sync_from_sgid_to_luoxue(self, *, sgid: str, api_key: str) -> SyncResult:
        ...
        try:
            ...
            if e.response.status_code == 403:
                raise Exception(
                    "落雪账号未开启第三方写入权限（allow_third_party_write_data），"
                    "请前往落雪查分器个人设置中开启。"
                )
            elif e.response.status_code == 401:
                raise Exception("落雪 API 密钥无效或已过期，请重新生成并绑定。")
            else:
    ...
    async def _upload_divingfish_from_details(self, details: list[dict], rating: int, token: str) -> SyncResult:
        """使用已获取的官方详情上传到水鱼"""
        scores = await self._official_details_to_scores(details)
        if not rating:
            rating = await self._rating_from_score_list(scores)
        await self.client.updates(
            self._identifier(credentials=token),
            scores,
            provider=self._divingfish_provider(),
        )
        return SyncResult(
    ...
    async def _upload_luoxue_from_details(self, details: list[dict], rating: int, api_key: str) -> SyncResult:
        """使用已获取的官方详情上传到落雪"""
        luoxue_scores = await self._official_details_to_luoxue_scores(details)
        # 过滤掉落雪未收录的曲目
        try:
            ...
            filtered_count = original_count - len(luoxue_scores)
            if filtered_count > 0:
                logger.warning("[MaimaiUpdater] filtered %s scores not found in luoxue song list", filtered_count)
        except Exception as e:
            logger.warning("[MaimaiUpdater] failed to get luoxue song list, skip filtering: %s", e)
        if not luoxue_scores:
            raise Exception("过滤后没有有效的成绩可上传")
        result_json = await self._upload_to_luoxue(luoxue_scores, api_key)
        if not result_json.get("success"):
            raise Exception(result_json.get("message", "落雪 API 返回失败"))
        marked_count = sum(1 for s in luoxue_scores if s.get("fc") or s.get("fs"))
